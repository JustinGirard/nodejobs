...
class JobDB():
    ...
    def __init__(self,db_path:str):    
        ...
        1. If `db_path` is None, raise an exception because a storage path is mandatory.
        2. Construct the full file path as `<db_path>/jobs.db`.
        3. Instantiate the underlying NoSQL wrapper (`nosqlite`) using that path,
           so subsequent calls will operate on “jobs.db” (created on first write).
    ...
    def update_status(self,job):
        ...
        1. Wrap the raw `job` dict in a validated `JobRecord` and call `.clean()` to
           strip out any extraneous fields.
        2. Perform an “upsert” on the “process_status” table:
           - If a record with the same `self_id` exists, update its fields.
           - Otherwise, insert a new record.
        3. Return whatever the underlying `nosqlite.execute(...)` returns (e.g., row count or error).
        ...
        Returns:
            Any:  The result of the upsert operation (often the new record’s ID or a status code).
        """        
    ...
    def job_logs(self,self_id):
        ...
        1. Query the “process_status” table for a record whose 'self_id' matches `self_id`.
        2. If no record is found, return two empty strings.
        3. Verify that the record contains both 'logdir' and 'logfile' fields; if missing,
        ...
        5. Attempt to open & read each file; if opening fails, return an error string instead.
        6. Return a tuple `(stdout_contents, stderr_contents)`.

        Parameters:
            self_id (str):  Unique identifier of the job whose logs should be fetched.

        Returns:
            (str, str):  The contents of stdout and stderr log files.  If the job or files
                         cannot be found, error messages are returned in place of file contents.
        """
    ...
    def list_status(self,filter=None):
        ...
        1. If `filter` is None, use an empty dictionary (match everything).
           Otherwise, validate and clean the filter via `JobFilter(filter, trim=True).clean()`.
        2. Execute a 'find' query on the “process_status” table using `clean_filter`.
        3. If the database returns an Exception object, re‐raise it.
        4. Assemble a dict mapping each job’s 'self_id' to its full record.
        5. Return this dict, which may be empty if no records match (or table is empty).
        ...
        Returns:
            dict:  { self_id: record_dict, … } for each job matching the filter.
                   If no jobs match, returns an empty dict.  If an error occurs
        ...
        jobs_by_id = {}
        for j in resp:
            j = JobRecord(j) # Crash the system if the DB is corrupt (contains invalid records)
